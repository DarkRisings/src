...
void song_update(void)
{
   ...
   /* do the global song, if any */
	...
	else
	{
		...
		victim = d->original ? d->original : d->character;
        	...
        	if ( d->connected == CON_PLAYING &&
             	     !IS_SET(victim->comm,COMM_NOSING) &&
             	     !IS_SET(victim->comm,COMM_QUIET) )
                     act_new("$t",d->character,buf,NULL,TO_CHAR,POS_SLEEPING,0);
            }
	}
    }
	...
	if (obj->value[0] < 0)
	{
	    sprintf(buf,"$p starts playing a sonnet by %s, called %s.",
	...
	sprintf(buf,"$p sings: '{g%s{x'",line);
                act(buf,room->people,obj,NULL,TO_ALL); 
    }
}
...
void do_play(CHAR_DATA *ch, char *argument)
{
    ...
    if (argument[0] == '\0')
    {
	send_to_char("Request what?\n\r",ch);
	return;
    }

    if (juke == NULL)
    {
	send_to_char("You see nothing to request by that name.\n\r",ch);
	return;
    }
    ...
    {
        send_to_char("The bard has too many requests at the moment.\n\r",ch);
        return;
    }
