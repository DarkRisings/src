...
MPEDIT(mpedit_show)
{
    ...
    sprintf(buf,
	    ...
	    "Assigned:   [%d]\n\r"
           "Code:\n\r%s\n\r",
           pMcode->vnum, pMcode->assigned, pMcode->code);
    send_to_char(buf, ch);
...
{
    AREA_DATA *pArea;
    MPROG_CODE *pMcode, *mpcode;
    BUFFER *buffer;
    char buf[MAX_STRING_LENGTH];
    int minvnum, maxvnum, i;
    bool found = FALSE;
    ...
    maxvnum = pArea->max_vnum;

    for( i = minvnum; i < maxvnum; i++ )
    {
        if ( (mpcode = get_mprog_index( i ) ) != NULL )
        {
            sprintf(buf, "%5d assigned %d times\n\r", 
                mpcode->vnum, mpcode->assigned );
            add_buf(buffer,buf);
            found = TRUE;
        }
    }
    ...
    if ( !found )
    {
        add_buf( buffer, "No mobprogs found.\n\r" );
    }
