	...
	strcpy( tmparg3, argument );
	    ...
	    *ch->desc->pString = str_dup( "" );
        ...
        if ( !str_cmp( arg1, ".r" ) )
        {
            ...
            if ( ( strlen( *ch->desc->pString ) 
               - strlen( arg2 ) 
               + strlen( arg3 ) ) >= ( MAX_STRING_LENGTH - 4 ) )
                send_to_char( "The resulting text may be too long.  "
                    "Try removing some text first.\n\r", ch );
            else
            {
                if ( strstr( *ch->desc->pString, arg2 ) != NULL )
                    sprintf( buf, "'%s' replaced with '%s'.\n\r", arg2, arg3 );
                else
                    sprintf(buf, "Substring '%s' not found.\n\r", arg2 );
                *ch->desc->pString =
...
/*          send_to_char( "Sedit help (commands on blank line):   \n\r", ch );
            ...
            send_to_char( ".li <num> <str>  - insert <str> after line <num>\n\r", ch );
	    send_to_char( ".lr <num> <str>  - replace line <num> with <str>\n\r", ch );
            send_to_char( "@                - exit editor          \n\r", ch ); */
/* new and improved */
send_to_char("Command to Type             What It Does                 \n\r", ch );
send_to_char("========================    =============================\n\r", ch );
send_to_char(".h                        - lists helpful editor commands\n\r", ch );
send_to_char(".r 'old junk' 'new junk'  - replaces the word or phrase in the first set of ''\n\r", ch );
send_to_char("                            with the word or phrase in the second set of ''\n\r", ch );
send_to_char(".s                        - shows the text you have written so far, with\n\r", ch );
send_to_char("                            numbered lines\n\r", ch );
send_to_char(".ld <#>                   - deletes line number <#>\n\r", ch );
send_to_char(".li <#> <newline>         - inserts <newline> before what is currently line\n\r", ch );
send_to_char("                            number <#>, making <newline> number <#>\n\r", ch );
send_to_char(".lr <#> <newline>         - erases line number <#> and replaces it with the\n\r", ch );
send_to_char("                            <newline>, making <newline> number <#>\n\r", ch );
send_to_char(".c                        - clears all the text you have written so far\n\r", ch );
send_to_char(".f                        - formats all the text you have written so far\n\r", ch );
send_to_char("@                         - exits the editor\n\r", ch );
            return;
        }
			...
			for ( hash = 0; hash < MAX_KEY_HASH; hash++ )
				for ( mob = mob_index_hash[hash]; mob; mob = mob->next )
					for ( mpl = mob->mprogs; mpl; mpl = mpl->next )
						...
						{
							sprintf( buf, "Code changed on mob %d.\n\r", mob->vnum );
							send_to_char( buf, ch );
							mpl->code = mpc->code;
						}
	}
...
{
/*     MSL x 2 to prevent overflows due to single-spacing after periods */
...
/* This squelches crashes in the event of bad things */
  if ( strlen( xbuf ) > MAX_STRING_LENGTH )
    xbuf[MAX_STRING_LENGTH-1] = '\0';

  return(str_dup(xbuf));
}

/* format_string() without the memory allocation bits */
void format_string_tmp( char *oldstring, char *newstring )
{
  char xbuf[MAX_STRING_LENGTH*2];
  char xbuf2[MAX_STRING_LENGTH*2];
  char *rdesc;
