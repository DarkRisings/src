...
{
/*  The original had MSLx2, probably because it was too
    buggy to not crash without the extra stringspace.  It
    shouldn't be necessary now, but it stays in as a safety
    precaution */
    static char strfix[MAX_STRING_LENGTH * 2];
    int oldstr = 0, newstr = 0/*, los*/;

/*  los = strlen(str); */
    while ( str[oldstr] != '\0' /*&& oldstr < (los-1) */)
    {
        if ( str[oldstr] != '\r' && str[oldstr] != '~' )
        {
...
/* The stock write of this function was buggy as shit and caused terrible
   memory corruption issues.  It's a surprise it didn't do more damage
   earlier, but I rewrote it above on June 19, 2006 (gkl) 
char *fix_string_old( const char *str )
{
    static char strfix[MAX_STRING_LENGTH * 2];
    int i;
    int o;
...
void save_mobile( FILE *fp, MOB_INDEX_DATA *pMobIndex )
{
    ...
    fprintf( fp, "%s ",		fwrite_flag( pMobIndex->affected_by3,	buf ) );
    ...
    if ((temp = DIF(race_table[race].aff3,pMobIndex->affected_by3)))
     	fprintf( fp, "F aff3 %s\n", fwrite_flag(temp, buf) );
 ...
 */
    ...
    switch ( pObjIndex->item_type )
    {
        ...
        case ITEM_FOUNTAIN:
            fprintf( fp, "%d %d '%s' %d '%s'\n",
		     ...
		     pObjIndex->value[3],
		     pObjIndex->value[4] < 0 ?
		     skill_table[-1*pObjIndex->value[4]].name :
    ...
    if (!IS_NULLSTR(pObjIndex->owner))
     fprintf(fp,  "O %s~\n",    pObjIndex->owner);

    /* TIME DOWN IS AN OBJECT TIMER FOR FADING OBJECTS */
    /* ARAMINA - 8/23/02 */
    if (pObjIndex->timedown > 0)
     fprintf(fp, "T %d\n", pObjIndex->timedown);
...
void save_rooms( FILE *fp, AREA_DATA *pArea )
{
    ...
    for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
    {
		...
		fprintf( fp, "%d ",             pRoomIndex->trans_flags );
...
void save_quests( FILE *fp, AREA_DATA *pArea )
{
    ...
    fprintf( fp, "#QUESTS\n" );
    for ( quest = pArea->quests; quest != NULL; quest = quest->next )
    {
        fprintf(fp, "Q %d", quest->completevnum );
        fprintf(fp,  " %s", fwrite_flag(quest->completeflag, buf) );
        fprintf(fp,  " %s", fwrite_flag(quest->flags, buf) );
        fprintf(fp,  " %s~\n", fix_string(quest->name) );
        for ( journal = quest->journal; journal != NULL; 
                                                journal = journal->next)
        {
            fprintf( fp, "J %d %d %s %s~\n",  journal->step,
                journal->vnum, fwrite_flag(journal->flag, buf),
                fix_string(journal->text) );
        }
    }
    fprintf( fp, "0\n\n\n\n" );
