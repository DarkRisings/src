...
void show_groups args( ( CHAR_DATA *ch, char *argument, bool onlyclass ) );
...
void do_gain(CHAR_DATA *ch, char *argument)
{
	...
	if (IS_NPC(trainer) && IS_SET(trainer->act_bits,ACT_GAIN))
	    break;
	...
	sprintf(buf, "%-19s %-5s %-19s %-5s %-19s %-5s\n\r",
...
void do_spells(CHAR_DATA *ch, char *argument)
{
	...
	||  (IS_SET(ch->shiftbits,SHIFT_POTENTIAL) &&
	    (level = skill_table[sn].were_level[ch->race]) < LEVEL_HERO + 1))
	&&  (fAll || level <= ch->level)
	...
	&&  ch->pcdata->learned[sn] > 0)
        {
            ...
            {
                if ( ch->level < level )
                    sprintf(buf, "%-20s n/a    n/a    ", skill_table[sn].name);
                else
                {
/*                  mana = UMAX(skill_table[sn].min_mana, 100/(2 + ch->level - level)); */
                    ...
                    {
                    sprintf(buf, "%-20s %3d   %3d%c    ",
                        skill_table[sn].name,
                        ( sn == gsn_veil ? ( ch->level * 10 ) : mana ),
                        1,
                        '%');
            ...
            else
            {
                if (ch->level < level)
                {
                    sprintf(buf,"%-19s n/a      ", skill_table[sn].name);
                }
                else
                {
/*                  mana = UMAX(skill_table[sn].min_mana, 100/(2 + ch->level - level)); */
                    mana = skill_table[sn].min_mana;
                    sprintf(buf,"%-19s  %3d mana  ", skill_table[sn].name,
                        ( sn == gsn_veil ? ( ch->level * 10 ) : mana ) );
                }
    ...
    if ( IS_DRMODE( ch ) )
       send_to_char(
       "Lv  Spell               Cost  Skill    Spell               Cost  Skill",ch );
    buffer = new_buf();
    for (level = 0; level < LEVEL_HERO + 1; level++)
      	if (spell_list[level][0] != '\0')
    ...
    free_buf(buffer);
    if ( global_config_aprilfools && ch->pcdata->aprilfools ) ch->pcdata->aprilfools = FALSE;
}
	 ...
	 /*   level = skill_table[sn].skill_level[ch->class]; */
	    ...
	    if (ch->level < level)
	    	...
	    	sprintf(buf,"%-18s n/a      ", skill_table[sn].name);
                sprintf(buf2,"{M%-17s {cn/a{x ", skill_table[sn].name);
             }
	    else
             {
                if ( !ch->pcdata->aprilfools )
                {
                    sprintf(buf,"%-18s %3d%%      ",skill_table[sn].name,ch->pcdata->learned[sn]);
/*                  sprintf(buf2,"{M%-17s {c%2d{x ",skill_table[sn].name,dr);*/
                }
                else
...
void list_group_costs(CHAR_DATA *ch)
{
	...
	&&  group_table[gn].rating[ch->class] > 0)
	{
	    sprintf(buf,"%-19s %-5d ",group_table[gn].name,
	...
	&&  skill_table[sn].rating[ch->class] > 0)
        {
            sprintf(buf,"%-19s %-5d ",skill_table[sn].name,
...
void list_group_chosen(CHAR_DATA *ch)
{
    ...
    sprintf(buf,"%-19s %-5s %-19s %-5s %-19s %-5s","group","cp","group","cp","group","cp\n\r");
    send_to_char(buf,ch);
    ...
    sprintf(buf,"%-19s %-5s %-19s %-5s %-19s %-5s","skill","cp","skill","cp","skill","cp\n\r");
    send_to_char(buf,ch);
...
void show_groups(CHAR_DATA *ch, char *argument, bool onlyclass)
{
    char buf[100];
    int gn,sn,col,i;
	...
	if (group_table[gn].spells[sn] == NULL )
            break;
        if ( onlyclass
        &&   group_lookup( group_table[gn].spells[sn] ) == -1
        &&   ( (i = skill_lookup(group_table[gn].spells[sn])) < 0 
        ||   skill_table[i].skill_level[ch->class] > LEVEL_HERO ) )
	    continue;
 ...
 * smaller multiplier = faster improvement
 */
void check_improve( CHAR_DATA *ch, int sn, bool success, int multiplier )
{
   ...
   /* ||  skill_table[sn].rating[ch->class] == 0*/
    ...
    /* Double improvements if actively levelling */
    if ( ch->pcdata->skillbonus
    &&   (int)(current_time - ch->pcdata->skillbonus) < 120 )
      multfact /= 2;

    /* check to see if the character has a chance to learn */
    chance = 10 * int_app[get_curr_stat(ch,STAT_INT)].learn;
    if ( sn == gsn_scrolls ) 
        chance = 2 * chance / ( multiplier * multfact );
    else
        chance /= multiplier * multfact;
...
int group_lookup( const char *name )
{
	...
	*/
	...
	if( LOWER( name[ 0 ] ) == LOWER( group_table[ gn ].name[ 0 ] ) &&
	    strstr( group_table[ gn ].name, name ) != NULL )
	  return gn;
    }
