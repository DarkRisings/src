    ...
    */
	...
	if ( !str_prefix( arg1, "begin" ) ) 
	{
		...
		for( d = descriptor_list; d != NULL; d = d->next )
		{
			if( d->connected == CON_PLAYING && !IS_NPC( d->character ) )
			{
				...
				{
					sprintf(buf, "%s and %s descend the stairs and take their place on a horse at either end of the arena.\n\r",  jouster1->name, jouster2->name);
					send_to_char(buf, d->character); 
				}
			}
		}
	...
	if ( !str_prefix( arg1, "continue" ) ) 
	{
		...
		if (jouster1 == NULL && jouster2 == NULL) {
			...
			else if (jouster1 == NULL || jouster2 == NULL) {
		...
		else if (jouster1->joust_ready == 2 && jouster2->joust_ready == 2)
		{
			...
			if (jouster1->joust_win == 0 && jouster2->joust_win == 0) 
			{
				...
				sprintf(buf, "As your horse charges toward the center of the arena, you %s and %s %ss!  It's anybody's game!\n\r",  
				...
				for( d = descriptor_list; d != NULL; d = d->next )
				{
					if( d->connected == CON_PLAYING && !IS_NPC( d->character ) )
					{
						...
						{
							sprintf(buf, "As %s's horse charges toward %s, %s %ss and %s %ss! It's anybody's game!{x\n\r",  
			...
			{
				sprintf(buf, "{GAs your horse charges toward the center of the arena, you %s and %s %ss! You win!{x\n\r",  
				jouster1->joust_attack, jouster2->name, jouster2->joust_attack);
				send_to_char(buf, jouster1); 
		
				sprintf(buf, "{RAs your horse charges toward the center of the arena, you %s and %s %ss! You lose!{x\n\r",  
				jouster2->joust_attack, jouster1->name, jouster1->joust_attack);
				send_to_char(buf, jouster2); 
